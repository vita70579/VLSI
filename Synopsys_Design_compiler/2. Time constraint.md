...
>由於我們要合成的對象是灰色方塊中的組合邏輯N，但要分析其時序約束條件必須將外部的時序電路同時考慮進去:
  >>- 由方塊圖可以看出約束式: Tclk-(Tcq+Tcomb_M+Tcomb_N) > Tsetup
  >>- 因此我們可以推導出合成組合邏輯N的約束條件: Tclk-(Tcq+Tcomb_M) > Tsetup+Tcomb_N，故DC會依照組合邏輯N的延遲條件優化。
  >>- 我們可以合理設置不確定時間(δ)與外部延遲(Tcq, Tcomb_M)，可以得到: Tcomb_N_max=Tclk+Tskew-(Tcq+Tcomb_M)-δ-Tsetup
  >>>![Image](https://github.com/vita70579/VLSI/raw/main/Image/im46.png)
...
>由於我們要合成的對象是灰色方塊中的組合邏輯S，但要分析其時序約束條件必須將外部的時序電路同時考慮進去:
  ...
  >>>![Image](https://github.com/vita70579/VLSI/raw/main/Image/im47.png)
